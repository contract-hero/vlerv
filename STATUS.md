...
- **Registered with Claude Code** at user scope (`claude mcp add vlerv --scope user`), `VLERV_MCP_ROOTS=/Users/valrov/workspace`. Its node id is `f378bc7f…`; `~/Library/Application Support/Vlerv/mcp/remote/peers.json` is still empty — no device has paired with the server yet.
...
- **`confirm_pairing` applies the scope the human named**, in both directions. A named scope replaces an already-trusted device's grant — re-pairing a `control` peer at `view-open` now actually narrows it. Omitting the scope names no grant, so an existing peer keeps what it has and a new one gets `view-open`. The rule lives in one place, `PeerStore::confirm` (the passive `upsert` path still never moves a grant); `remote_pair_confirm` in the app uses it too.
...
- **MCP pairing not done**: the server is built and registered, but no device trusts it yet. Pairing needs the phone in hand — `pair_device` → open the link on the phone → compare the six words → the phone grants `Claude Code @ …` the `control` scope → `confirm_pairing`. Runbook in `REMOTE-HANDOFF.html`.
